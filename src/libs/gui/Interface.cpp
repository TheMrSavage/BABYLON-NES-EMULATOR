...
// TODO: Solve the input lag (*very* laggy)
    ...
    while (true) {
        for (int i = 0; i < this->screens_vec.size(); i++) {
            ...
            while (SDL_PollEvent(&event)) {
                ...
                if ( (event.type == SDL_EVENT_QUIT) || (event.type == SDL_EVENT_WINDOW_CLOSE_REQUESTED && event.window.windowID == SDL_GetWindowID(sdl_data->window)) ) {
                    return;
                }
            }
            ...
            ImGui::Render();
            SDL_SetRenderScale(sdl_data->renderer, this->imguiIO.DisplayFramebufferScale.x, this->imguiIO.DisplayFramebufferScale.y);
