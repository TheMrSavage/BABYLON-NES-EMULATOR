...
bool execute6502Test(const nlohmann::json& test) {
    ...
    for (const auto& memoryValues : test["initial"]["ram"]) {
        std::cout << "[+] Writting to memory at " << memoryValues[0] << " value: " << memoryValues[1] << std::endl;
    ...
    if (cpuPFlag != finalPFlag) {
        std::cout << "[-] Cpu PFlag != Final PFlag: (" << (int)cpuPFlag << ", " << (int)finalPFlag << ")" << std::endl;
        std::cout << "[-] Failed test: " << testName << std::endl;
        return false;
    }
...
void Cpu6502TestsOfficialInstructions() {
    const std::string Cpu6502TestsOfficialInstructionsPath = "testcase/6502/official/";

    for (const auto& entry : std::filesystem::directory_iterator(Cpu6502TestsOfficialInstructionsPath)) {
...
void Cpu6502Tests() {
    Cpu6502TestsOfficialInstructions();
    Cpu6502NESTests();
}
